...
class SilverTransformation:
    ...
    def process_frentes_members(self):
        ...
        print("--- INICIANDO TRANSFORMAÇÃO SILVER: FRENTES PARLAMENTARES ---")
        ...
        if not path_membros or not path_deputados:
            print("[ERROR] Arquivos da camada Bronze ausentes para frentes/deputados.")
            return
        ...
        # English: Executing the Left Join
        # Português: Executando o cruzamento Left Join
        df_silver = pd.merge(
            df_membros_clean, 
            df_dep_clean, 
            on='id', 
        ...
        print(f"[SUCCESS] Silver de Frentes consolidada. {len(df_silver)} registros processados.")
